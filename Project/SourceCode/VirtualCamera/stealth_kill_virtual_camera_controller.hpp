...
class StealthKillVirtualCameraController final : public IVirtualCameraController
{
public:
	StealthKillVirtualCameraController();
	~StealthKillVirtualCameraController();
...
private:
	#pragma region Event
	void SetStealthKillerModelHandle	(const StealthKillEvent&	event);
	void SetStealthKillableModelHandle	(const OnStealthKillEvent&	event);
	#pragma endregion
...
private:
	static constexpr VECTOR kFollowOffset		= { 0.0f,  10.0f, -60.0f };
	static constexpr VECTOR kTrackedObjOffset	= { 5.0f, -10.0f,   0.0f };
...
private:
	...
	std::shared_ptr<Modeler>		m_stealth_killer_modeler;		// ステルスキルを行う側のモデルハンドル
	std::shared_ptr<Modeler>		m_stealth_killable_modeler;		// ステルスキルされる側のモデルハンドル
