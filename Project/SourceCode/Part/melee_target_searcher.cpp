...
void MeleeTargetSearcher::SelectDownedCharacter(std::shared_ptr<IMeleeAttackable>& melee_attacker)
{
	...
	//		  そうでない場合は、カメラのforwardに近い順でソート
	//		  最終的に最も優先度が高いメレー対象のみを残す
	for (const auto& candidate : melee_attacker->GetVisibleDownedCharacter())
	{
		if (candidate.camera_diff_angle < kPrioritySwitchAngle * math::kDegToRad)
		{
			distance.emplace_back(candidate);
	...
	{
		std::ranges::sort(distance, {}, &MeleeCandidateData::distance_to_camera);
		melee_attacker->AddTopPriorityVisibleDownedCharacter(distance.front().target_obj_handle);
		return;
	}

	// 角度でソート
	std::ranges::sort(angle, {}, &MeleeCandidateData::camera_diff_angle);
	melee_attacker->AddTopPriorityVisibleDownedCharacter(angle.front().target_obj_handle);
}
