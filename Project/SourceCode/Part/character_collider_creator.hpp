...
public:
	...
	void CreateCollisionAreaTrigger	(PhysicalObjBase* physical_obj, const float radius);
	...
	void CalcCollisionAreaTriggerPos(std::shared_ptr<Modeler>& modeler, const std::unordered_map<ColliderKind, std::shared_ptr<Collider>>& collider, const VECTOR& offset);
