...
void EffectManager::OnNotify(ObjBase& obj, const EventKind event_kind)
{
	switch (event_kind)
	{
	case EventKind::kShot:
		CreateGunEffect(obj, event_kind);
		CreateBulletEffect(obj, event_kind);
		break;
...
void EffectManager::CreateGunEffect(ObjBase& obj, const EventKind event_kind)
{
...
void EffectManager::CreateBulletEffect(ObjBase& obj, const EventKind event_kind)
{
	const auto pool = ObjectPoolHolder::GetInstance()->GetObjectPool(ObjectPoolName.PLAY_SCENE_EFFECT_POOL);
	std::shared_ptr<ObjBase> effect_obj = nullptr;
	std::shared_ptr<Effect>	 effect		= nullptr;
