...
void AnimatorBase::AddAnimHandle(const int kind, const std::string& file_path, const int index, const std::string& tag, const float play_speed, const bool is_loop, const bool is_self_blend, const float landing_paly_rate)
{
	// 上書き不可
	if (m_anim_data.contains(kind)) { return; }

	int anim_handle = HandleKeeper::GetInstance()->LoadHandle(HandleKind::kAnim, file_path);
	if (anim_handle != -1)
	{
		m_anim_data[kind] = AnimKindData(anim_handle, index, tag, play_speed, is_loop, is_self_blend, landing_paly_rate);
	}
}
...
void AnimatorBase::SetupStaticFrame()
{
...
void AnimatorBase::DivideFrame(const TCHAR* upper_body_end_frame)
{
	const int model_handle = m_result_modeler->GetModelHandle();
...
{
	if (m_anim_data.contains(prev_time_kind_data.kind))
	{
...
bool AnimatorBase::CanAttachAnim(const int next_kind, const BodyKind body_kind)
{
	...
	if (!m_anim_data.contains(next_kind))
	{
		return false;
	}
