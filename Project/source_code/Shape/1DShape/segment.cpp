...
Segment::Segment(const VECTOR& begin_pos, const VECTOR& end_pos):
	...
	m_length    (0.0f)
{
	MakeSegment(m_begin_pos, m_end_pos);
}
...
Segment::Segment(const VECTOR& begin_pos, const VECTOR& dir, float length) :
	...
	m_length    (length)
{
	MakeSegment(m_begin_pos, m_dir, m_length);
}
...
void Segment::MakeSegment(const VECTOR& begin_pos, const VECTOR& end_pos)
{
	m_begin_pos = begin_pos;
	m_end_pos   = end_pos;
...
void Segment::MakeSegment(const VECTOR& begin_pos, const VECTOR& dir, float length)
{
	m_begin_pos = begin_pos;
...
void Segment::Draw(int color)const
{
