...
float math::GetAdjacentLengthIsoscelesRightTriangle(float hypotenuse_length)
{
    return sqrt(hypotenuse_length * hypotenuse_length * 0.5f);
}

float math::GetHypotenuseLengthIsoscelesRightTriangle(float adjacent_length)
{
    return sqrt(adjacent_length * adjacent_length + adjacent_length * adjacent_length);
}

float math::GetAdjacentLengthRightTriangle(float opposite_length, float hypotenuse_length)
{
    return sqrt(hypotenuse_length * hypotenuse_length - opposite_length * opposite_length);
}

float math::GetHypotenuseLengthRightTriangle(float adjacent_length, float opposite_length)
{
...
{
    float radian = VDot(vector1, vector2) / (VSize(vector1) * VSize(vector2));
...
float math::GetAngleBetweenTwoVector(const VECTOR& vector1, const VECTOR& vector2)
{
    ...
    float length1 = VSize(vector1);
    float length2 = VSize(vector2);
...
{
    VECTOR  centroid_triangle1 = GetCentroidOfATriangle(pos1, pos2, pos3);
    VECTOR  centroid_triangle2 = GetCentroidOfATriangle(pos3, pos4, pos1);
    Line    line1(centroid_triangle1, v3d::GetNormalizedVector(centroid_triangle2 - centroid_triangle1));

    VECTOR  centroid_triangle3 = GetCentroidOfATriangle(pos2, pos3, pos4);
    VECTOR  centroid_triangle4 = GetCentroidOfATriangle(pos4, pos1, pos2);
    Line    line2(centroid_triangle3, v3d::GetNormalizedVector(centroid_triangle4 - centroid_triangle3));

    // 四角形の時点でnulloptを返すことはないが、
    // 引数が正常でなかった場合nulloptである可能性があるため判定を行う
    auto   intersection = GetIntersectionLineAndLine(&line1, &line2);
...
std::optional<VECTOR> math::GetIntersectionLineAndLine(const Line* line1, const Line* line2)
{
    ...
    VECTOR p1p2         = line2->GetPos() - line1->GetPos();
    VECTOR cross_v      = VCross(line1->GetDir(), line2->GetDir());
    float  cross_length = VDot(cross_v, cross_v);

    if (cross_length == 0.0f) { return std::nullopt; }

    float t1 = VDot(VCross(p1p2, line1->GetDir()), cross_v) / cross_length;
    float t2 = VDot(VCross(p1p2, line2->GetDir()), cross_v) / cross_length;

    VECTOR p1 = line1->GetPoint(t1);
    VECTOR p2 = line2->GetPoint(t2);
...
{
    VECTOR a = segment->GetBeginPos();
    VECTOR b = segment->GetEndPos();
    VECTOR ab = b - a;
    VECTOR pa = a - plane->GetPos();
    VECTOR pb = b - plane->GetPos();
    VECTOR n = plane->GetNormalVector();

    if ((VDot(pa, n) >= 0 && VDot(pb, n) <= 0) || (VDot(pa, n) <= 0 && VDot(pb, n) >= 0))
    {
        float dot_pa_n = std::abs(VDot(pa, n));
        float dot_pb_n = std::abs(VDot(pb, n));
...
bool math::IsSameLine(const Line* line1, const Line* line2)
{
    ...
    VECTOR direction = line2->GetDir() - line1->GetDir();
...
{
    float distance1 = GetDistancePointToPlane(segment->GetBeginPos(), plane);
    float distance2 = GetDistancePointToPlane(segment->GetEndPos(), plane);
...
{
    VECTOR p = GetClosestPosOnPlaneFromSegment(plane, segment);
    float distance = GetDistanceSegmentToPlane(segment, plane);

    VECTOR closest_pos = p + plane->GetNormalVector() * distance;
...
{
    VECTOR v = point - plane->GetPos();
    float angle = GetAngleBetweenTwoVector(plane->GetNormalVector(), v);
...
float math::GetDistancePointToSegment       (const VECTOR&      point,      const Segment*  segment,    VECTOR& h, float& t)
{
    ...
    VECTOR seg_dir = segment->GetEndPos() - segment->GetBeginPos();
    Line   line = Line(segment->GetBeginPos(), seg_dir);
    float  distance = GetDistancePointToLine(point, &line, h, t);

    VECTOR a = segment->GetBeginPos() - point;
    VECTOR b = segment->GetBeginPos() - segment->GetEndPos();
    VECTOR c = segment->GetEndPos() - point;
    VECTOR d = segment->GetEndPos() - segment->GetBeginPos();
...
{
    float distance1 = GetDistancePointToTriangle(point, square->GetTriangle(0));
    float distance2 = GetDistancePointToTriangle(point, square->GetTriangle(1));
...
float math::GetDistanceLineToLine           (const Line*        line1,      const Line*     line2,      VECTOR& h1, VECTOR& h2, float& t1, float& t2)
{
    ...
    // ねじれ関係であった場合
    float  dot_v1v1 = VDot(line1->GetDir(), line1->GetDir());
    float  dot_v1v2 = VDot(line1->GetDir(), line2->GetDir());
    float  dot_v2v2 = VDot(line2->GetDir(), line2->GetDir());
    VECTOR p21p11 = VSub(line1->GetPos(), line2->GetPos());
...
float math::GetDistanceSegmentToSegment     (const Segment*     segment1,   const Segment*  segment2,   VECTOR& h1, VECTOR& h2, float& t1, float& t2)
{
    ...
    if (segment1->GetLength() <= 0.0f || segment2->GetLength() <= 0.0f)
    {
        ...
        if (segment1->GetLength() <= 0.0f && segment2->GetLength() <= 0.0f)
        {
            float distance = VSize(segment2->GetBeginPos() - segment1->GetBeginPos());
        ...
        // 線分2が縮退している場合
        float distance = GetDistancePointToSegment(segment2->GetBeginPos(), segment1, h1, t1);
        h2 = segment2->GetBeginPos();
        t2 = 0.0f;
...
{
    VECTOR v1 = plane->GetPos() - segment->GetBeginPos();
    VECTOR v2 = plane->GetPos() - segment->GetEndPos();
...
{
    float distance1 = GetDistanceSegmentToTriangle(segment, square->GetTriangle(0));
    float distance2 = GetDistanceSegmentToTriangle(segment, square->GetTriangle(1));
...
{
    float distance = GetDistanceSegmentToPlane(capsule->GetSegment(), plane) - capsule->GetRadius();
...
{
    float distance1 = GetDistanceTriangleToTriangle(triangle, square->GetTriangle(0));
    float distance2 = GetDistanceTriangleToTriangle(triangle, square->GetTriangle(1));
...
{
    float distance1 = GetDistanceTriangleToSquare(square1->GetTriangle(0), square2);
    float distance2 = GetDistanceTriangleToSquare(square1->GetTriangle(1), square2);
...
{
    float distance = GetDistanceSegmentToSquare(capsule->GetSegment(), square) - capsule->GetRadius();
    return distance < 0.0f ? 0.0f : distance;
}
#pragma endregion
