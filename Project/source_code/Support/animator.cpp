...
Animator::Animator(std::shared_ptr<Modeler> modelr, float blend_speed) : 
...
void Animator::AttachAnim(int next_kind)
{
	...
	m_time_state_data.at(TimeState::kCurrent).attach_index = MV1AttachAnim(m_modelr->GetModelHandle(), 1, m_kind_data.at(next_kind).anim_handle, TRUE);
...
void Animator::DetachAnim(TimeState time_state)
{
	...
	{
		MV1DetachAnim(m_modelr->GetModelHandle(), m_time_state_data.at(time_state).attach_index);
		m_time_state_data.at(time_state).attach_index = -1;
	}
}

void Animator::SetAnimState()
{
...
void Animator::SetPlayStartTime()
{
	if (m_kind_data.count(m_time_state_data.at(TimeState::kPrev).kind))
	{
		...
		{
			float current_total_t = MV1GetAttachAnimTotalTime(m_modelr->GetModelHandle(), m_time_state_data.at(TimeState::kCurrent).attach_index);
			float prev_total_t	  = MV1GetAttachAnimTotalTime(m_modelr->GetModelHandle(), m_time_state_data.at(TimeState::kPrev).attach_index);
...
void Animator::Play()
{
	for (auto& [state_t, data] : m_time_state_data)
	{
		...
		{
			float total_t = MV1GetAttachAnimTotalTime(m_modelr->GetModelHandle(), data.attach_index);
			...
			MV1SetAttachAnimTime(m_modelr->GetModelHandle(), data.attach_index, data.play_timer);
			MV1SetAttachAnimBlendRate(m_modelr->GetModelHandle(), data.attach_index, blend_r);
		}
	}
}
